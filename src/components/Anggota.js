...
export default function Anggota() {
  ...
  useEffect(() => {
    async function fetchMembers() {
      try {
        const q = query(collection(db, "anggota"), orderBy("urutan", "asc"));
        ...
        {loading ? (
          ...
          <div className={styles.grid}>
            {members.map((member) => {
              ...
              const fotoSrc = member.foto
                ? `/image/anggota/${member.foto.trim()}.jpg`
                : null;
                  ...
                  <div className={styles.cardFront}>
                    <div
                      className={`${styles.avatarWrapper} ${
                        fotoSrc ? styles.avatarPhoto : styles[`avatar--${color}`]
                      }`}
                    >
                      {fotoSrc ? (
                        <AnggotaPhoto
...
// Sub-component: handles photo with fallback to initials
function AnggotaPhoto({ src, name, initials, color, styles }) {
